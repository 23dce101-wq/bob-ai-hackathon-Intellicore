...
import { effectiveCranes, HORIZON } from "./schedule";
import type {
  Berth,
  PortStatePayload,
  ScenarioEvent,
...
function assignVesselsToBerths(berths: Berth[], vessels: Vessel[]): void {
  ...
  for (const vessel of unassigned) {
    ...
    const base = vessel.type === "Container" ? 8 : vessel.type === "Bulk" ? 15 : 11;
    const hours = Math.max(2, Math.round(base * (3 / Math.max(1, cranes))));
...
export function buildPortState(input: ScenarioInput = {}): PortStatePayload {
  ...
  // Only vessels from user input (CSV import, manual entry)
