...
export function PortProvider({ children }: { children: ReactNode }) {
  ...
  const importVessels = useCallback((vessels: {
    ...
    if (toImport.length > 0) {
      const API = (import.meta.env.VITE_API_URL || "").replace(/\/+$/, "");
      fetch(`${API}/api/vessels/import`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vessels: toImport }),
  ...
  const resetAll = useCallback(() => {
    ...
    fetch(`${API}/api/vessels`, {
      method: "DELETE",
    }).catch((err) => console.warn("[MongoDB Sync] Failed to sync vessel reset:", err));
  }, []);

  const applyOptimization = useCallback(async () => {
    const API = (import.meta.env.VITE_API_URL || "").replace(/\/+$/, "");
    try {
      const res = await fetch(`${API}/api/vessels/apply-optimization`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ events, customVessels }),
      });
      if (!res.ok) {
      ...
      return {
        ...
        message: result.message || `Applied ${result.applied_count} move(s)`,
      };
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      console.error("[Apply Optimization] Error:", errorMsg);
      return {
        success: false,
        appliedCount: 0,
