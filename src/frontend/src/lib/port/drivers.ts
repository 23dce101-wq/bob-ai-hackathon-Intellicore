...
export function computeDrivers(
  ...
  // Check yard block saturation bottleneck
  const saturatedYards = berths.filter((b) => b.yard_saturation_pct >= 80);
  if (saturatedYards.length) {
    const worst = [...saturatedYards].sort((a, b) => b.yard_saturation_pct - a.yard_saturation_pct)[0]!;
    drivers.push({
      title: "Yard space saturation slowing RTG discharge",
      detail: `${saturatedYards.map((b) => `${b.yard_block_id} (${b.yard_saturation_pct}% full)`).join(", ")} exceeded critical density threshold (>80%), causing container stack shuffling and chassis wait times.`,
      weight: Math.min(95, Math.round(worst.yard_saturation_pct * 0.95)),
    });
  }
